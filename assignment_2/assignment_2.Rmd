...
a1.fited <- a1 %>% predict(., type="response") %>% signif(.,0)
...
# Something is wrong here because all errors are positive...
plot.df <- earthQ %>%
  mutate(eartScot.pred.a1 = a1 %>% predict(., type="response")) %>%
  dplyr::select(eartScot.pred.a1, eartrUK, decade) %>%
  mutate(eartScot=eartScot.pred.a1) %>% 
  dplyr::select (eartScot, eartrUK, decade)

earthQ %>%
  dplyr::select(eartScot,eartrUK, decade) %>%
  bind_rows(.,plot.df) %>%
  mutate(ud=factor(c(rep("actual",12), rep("a1.prediction",12)))) %>%
  ggplot(., aes(decade, eartScot, color=ud)) +
  geom_point(size=2) +
  labs(title="GLM fit",
       color=NULL) +
  geom_line()
