...
```{r helper}
b.fit.transform <- function(inits=NULL, data, model, chains=3, burnin, var.names, total){
...
test <- b.fit.transform(inits=inits,
                data=b1.data,
                model=model.s,
                chains=5,
                burnin=7000,
...
d1 <- fire.rm.d2 %>% ggplot(aes(x=total.fires, y=as.factor(region.month), fill=Region)) +
  geom_density_ridges() +
  ...
  labs(y="Region-month", x="Total Fires")

d2 <- fire.rm.d3 %>% ggplot(aes(x=total.fires, y=as.factor(region.month), fill=Region)) +
  geom_density_ridges() +
  geom_segment(data=fire.rm.oj,
               aes(x=name, xend=name,
                   y=as.numeric(as.factor(region.month)),
...
d3 <- fire.rm.e1 %>% ggplot(aes(x=total.fires, y=as.factor(region.month), fill=Region)) +
  geom_density_ridges() +
  geom_segment(data=fire.rm.oj,
               aes(x=name, xend=name,
                   y=as.numeric(as.factor(region.month)),
