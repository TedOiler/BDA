...
\item
Explanation: Finally, using our original model from part (b), we will predict the prices for the three next years. Firstly 3 new rows where created and binded to the dataset with NA on the column of price for prediction. The values for the features where the average of the columns, except for the transaction date column, which was 2014, 2015 and 2016 for the 3 rows (the years we want to predict) and the house age, which was calculated as 2020-(transaction date).
The mean values the three years are: 2938.587, 2943.112, 2947.637. This prices are not correct however, and further root cause analysis needs to be conducted in order to understand why these values have been produced. Below is the final Posterior predictive Density produces by the final model.
